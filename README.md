...
- The loggers are `*plumber.Logger` instead of `*logrus.Logger` and `*logrus.Entry`, and they keep their printf methods. `LogLevel` is an enum of its own that is parsed with `ParseLogLevel`, `WithField` and `WithFields` become `With`, and `SetFormatter` becomes `SetLoggerOptions`.
